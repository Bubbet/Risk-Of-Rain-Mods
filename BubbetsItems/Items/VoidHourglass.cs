...
namespace BubbetsItems.Items
{
	public class VoidHourglass : ItemBase
	{
		protected override void MakeTokens()
		{
			...
			AddToken("VOIDHOURGLASS_DESC", "{0:0%} chance to duplicate damage over time procs. " + "Corrupts all Abundant Hourglasses.".Style(StyleEnum.Void));
		...
		private void DotInflicted(DotController dotcontroller, ref InflictDotInfo inflictDotInfo)
		{
			...
			for (var i = 0; i < guarenteed; i++)
				dotcontroller.AddDot(inflictDotInfo.attackerObject, inflictDotInfo.duration, inflictDotInfo.dotIndex, inflictDotInfo.damageMultiplier, inflictDotInfo.maxStacksFromAttacker, inflictDotInfo.totalDamage);
