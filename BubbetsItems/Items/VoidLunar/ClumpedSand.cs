...
namespace BubbetsItems.Items.VoidLunar
{
	public class ClumpedSand : ItemBase
	{
		protected override void MakeTokens()
		{
			...
			AddToken(name + "_DESC_SIMPLE", "All attacks will " + "hit for an additional 1 ".Style(StyleEnum.Utility) + "(+1 per stack)".Style(StyleEnum.Stack) +" times".Style(StyleEnum.Utility) + " for " + "50% base damage".Style(StyleEnum.Damage) + ", but" + " reduces health regeneration by -3/s".Style(StyleEnum.Health) + " (-3/s per stack)".Style(StyleEnum.Stack) + ". " + convert);
